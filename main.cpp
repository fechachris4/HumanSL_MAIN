...
// CLEAN UP : This part is to configure the kinova arm settings if action is 
//            manually stopped, otherwise an error will appear next time
//            we try to move the arm, which requires a full arm restart to fix.
//            Leave as is
...
int main(){
    ...
    // Load task parameters from configuration file
    std::string task_params_path = "../config/task_parameters.conf";
    if (!TaskParameters::loadFromFile(task_params_path)) {
        std::cerr << "Warning: Failed to load task parameters from " << task_params_path << std::endl;
        std::cerr << "Using default values defined in the code." << std::endl;
    } else {
        std::cout << "Successfully loaded task parameters from " << task_params_path << std::endl;
    }
    ...
    // These vaiables stores informations that gets passes around different threads
    // Is used for both storing of data into logs and control of robot action flows.

    // For thread-safe vicon and kinova-joint data monitoring
    ...
    // Redundant buffers for monitoring syncing purposes
    // These are what get recorded to the logs
    ...
    // Custom Object that encapsulates the properties of a kinova arm
    ...
    // Updates current left/right arm joint's pos/vel/torque
    // Updates Vicon readings 
    ...
    // Initial state logged and printed to CLI at run-time
    {
    ...
    // Initialize the joint trajectory buffer
    ...
    // LOGGING THREAD
    // Starts a separate thread that captures current states everytime
    // vicon frame updates (so records at default 100Hz)
    // Logs the info into a .csv file
    ...
    // ARM EXECUTION THREAD
    // Continuously monitors the trajectory buffer and executes accordingly
    // Other buffer variables modifies robot behavior on the fly
    ...
    // Monitors current state, plans new trajectory segment / modifies behavior 
    // if certain criteria fulfilled, very specific to my project 
    // you probably wont need it if going for real-time predictive control 
