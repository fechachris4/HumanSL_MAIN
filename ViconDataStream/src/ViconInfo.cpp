...
void updateTubeInfo(TubeInfo& tube_info, std::vector<MarkerData>& tube_tip, std::vector<MarkerData>& tube_end, std::vector<MarkerData>& tube_mid){
    ...
    auto find_circum_center = [&](std::vector<Eigen::Vector3d> three_tube_points){

        if(three_tube_points.size() < 3){
            return Eigen::Vector3d(0,0,0);
        }

        Eigen::Vector3d v1 = three_tube_points[1] - three_tube_points[0];
        Eigen::Vector3d v2 = three_tube_points[2] - three_tube_points[0];
        ...
        if (std::abs(d) < 1e-10) {
            ...
            world_center = (three_tube_points[0] + three_tube_points[1] + three_tube_points[2]) / 3.0;
        } else {
            ...
            // Convert back to 3D
            world_center = three_tube_points[0] + ux * u1 + uy * u2;
        }
        return world_center;
    };

    Eigen::Vector3d tube_tip_center = find_circum_center(tube_tip_pts);
    Eigen::Vector3d tube_mid_center = find_circum_center(tube_mid_pts);
    Eigen::Vector3d tube_end_center = find_circum_center(tube_end_pts);


    TubeInfo result;
    
    // Calculate direction vector based on occlusion status
    if(tube_tip_center.x() != 0 && tube_end_center.x() != 0){
        result.direction = (tube_end_center - tube_tip_center).normalized();
        result.centroid = ((tube_end_center + tube_tip_center) / 2.0 );

    } else if(tube_tip_center.x() != 0 && tube_mid_center.x() != 0){
        result.direction = (tube_mid_center - tube_tip_center).normalized();
        result.centroid = ((tube_mid_center + tube_tip_center) / 2.0 );

    } else if(tube_mid_center.x() != 0 && tube_end_center.x() != 0){
        result.direction = (tube_end_center - tube_mid_center).normalized();
        result.centroid = ((tube_end_center + tube_mid_center) / 2.0 );
    }
    else{
        result.direction = Eigen::Vector3d(0,0,1);  // faulty value, will be skipped when detected later
        result.centroid =  Eigen::Vector3d(0,0,0);
    }
...
void updateTargetInfo(gtsam::Point3& target_info, std::vector<MarkerData>& target){
    ...
    for(auto marker : target){
        if(marker.occluded){return;}
        if(marker.x == 0){return;}
        if(marker.y == 0){return;}
        if(marker.z == 0){return;}
    }
...
void updateViconInfo(ViconInterface& vicon, gtsam::Pose3& left_base, gtsam::Pose3& right_base, TubeInfo& tube_info, 
    ...
    static std::deque<gtsam::Pose3> ref_to_left_array;
    static std::deque<gtsam::Pose3> ref_to_right_array;
    ...
    static gtsam::Pose3 avg_ref_base;
    ...
    ref_base_data.push_back(vicon.getMarkerPosition("human_ref_base1"));
    ref_base_data.push_back(vicon.getMarkerPosition("human_ref_base2"));
    ref_base_data.push_back(vicon.getMarkerPosition("human_ref_base3"));
    ref_base_data.push_back(vicon.getMarkerPosition("human_ref_base4"));
    ...
    if(tube_info_array.size() >= 20){
        ...
        if(counter == 6){
            ...
            avg_centroid /= tube_info_array.size();
            avg_direction /= tube_info_array.size();
            avg_direction.normalize();  // Ensure unit vector
            avg_length /= tube_info_array.size();
    ...
    gtsam::Pose3 ref_base_current;
    gtsam::Pose3 ref_to_left_current;
    gtsam::Pose3 ref_to_right_current;

    if (!left_base_occluded && !right_base_occluded && !ref_base_occluded && !set_ref_to_base){
        ...
        ref_to_right_current = ref_base_current.inverse() * right_base_current; 
        ref_to_left_current = ref_base_current.inverse() * left_base_current; 
    ...
    if (!ref_base_occluded){
        ...
        if (ref_base_array.size() >= 10) {
            // Calculate current average rotation for left base

            gtsam::Rot3 avg_ref_rotation = avg_ref_base.rotation();
    ...
    if (ref_base_array.size() >= 5) {
        ...
        avg_ref_base = gtsam::Pose3(avg_ref_rotation, avg_ref_translation);

        if(set_ref_to_base){
            left_base = avg_ref_base * ref_to_left;
            right_base = avg_ref_base * ref_to_right;
        }
    }
    ...
    if (ref_to_left_array.size() >= 100 && !set_ref_to_base) {
        ...
        // This is to filter out outliers from the calibration set
        ...
        double left_mad_x = getMedian(left_abs_dev_x) * 1.4826; // Scale factor for normal distribution
        double left_mad_y = getMedian(left_abs_dev_y) * 1.4826;
        double left_mad_z = getMedian(left_abs_dev_z) * 1.4826;

        // Filter ref_to_left_array based on MAD threshold (2 MADs for more conservative filter)
        double mad_threshold = 2.0;
        std::deque<gtsam::Pose3> filtered_ref_to_left_array;
        for (const auto& pose : ref_to_left_array) {
            gtsam::Point3 translation = pose.translation();
        ...
        std::deque<gtsam::Pose3> filtered_ref_to_right_array;
        for (const auto& pose : ref_to_right_array) {
            gtsam::Point3 translation = pose.translation();
        ...
        avg_left_translation = avg_left_translation / ref_to_left_array.size();
        avg_left_rotation_vector /= ref_to_left_array.size();
        gtsam::Rot3 avg_left_rotation = gtsam::Rot3::Expmap(avg_left_rotation_vector);
        
        ref_to_left = gtsam::Pose3(avg_left_rotation, avg_left_translation);

        // Calculate average ref_to_right transform
        ...
        avg_right_translation = avg_right_translation / ref_to_right_array.size();
        ...
        ref_to_right = gtsam::Pose3(avg_right_rotation, avg_right_translation);
...
void calibrate_base_frame(ViconInterface& vicon, gtsam::Pose3& left_base, gtsam::Pose3& right_base, std::vector<double>& left_conf, std::vector<double>& right_conf, 
                            DHParameters& dh_params, std::shared_mutex& vicon_data_mutex, std::shared_mutex& joint_data_mutex){
    ...
    static std::deque<gtsam::Pose3> ref_to_right_array;
    static std::deque<gtsam::Pose3> ref_base_array;
    static std::deque<gtsam::Pose3> ref_ee_array;
    ...
    std::vector<MarkerData> ref_base_data;
    std::vector<MarkerData> ref_ee_data;
    ...
    ref_ee_data.push_back(vicon.getMarkerPosition("ref_ee1"));
    ref_ee_data.push_back(vicon.getMarkerPosition("ref_ee2"));
    ref_ee_data.push_back(vicon.getMarkerPosition("ref_ee3"));
    ref_ee_data.push_back(vicon.getMarkerPosition("ref_ee4"));
    ...
    for (const auto& marker : ref_ee_data) {
        if (marker.occluded) {
            ref_ee_occluded = true;
            break;
        }
    }
    ...
    if(set_ref_to_base){
        if(!ref_ee_occluded){
            gtsam::Pose3 ref_ee_dud = updatePoseInfo2(ref_ee_data, ref_ee_data.back());
            gtsam::Rot3 ref_ee_rot = ref_ee_dud.rotation() * gtsam::Rot3().Rx(-M_PI/2);
            ref_ee_current = gtsam::Pose3(ref_ee_rot, ref_ee_dud.translation());

            gtsam::Vector q_cur_right(7);
            for(int i = 0; i<7; i++){
                q_cur_right(i) = right_conf[i] * (M_PI/180);
            }
            gtsam::Pose3 right_ee_current = forwardKinematics(dh_params, q_cur_right, right_base);

            std::cout << "Right EE Measured: " << ref_ee_current << "\n";
            std::cout << "Right EE Derived:  " << right_ee_current << "\n";
        }
    }
}
