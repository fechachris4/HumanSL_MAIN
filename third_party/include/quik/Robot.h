...
public:
	...
	// @brief	Eigen::Matrix<double,DOF,4> DH: A DOFx4 array of the DH params in the following order:
	//         [a_1  alpha_1    d_1   theta_1;
	//          :       :        :       :    
	//          an   alpha_n    d_n   theta_n ];
	//
    Eigen::Array<double,DOF,4> DH;

	// @brief Eigen::Vector<JOINTTYPE_t,DOF> linkTypes: A vector of link types. Specify JOINT_REVOLUTE
	// or JOINT_PRISMATIC
	Eigen::Vector<quik::JOINTTYPE_t,DOF> linkTypes;
    
	// @brief Eigen::Vector<double,6> Qsign: A vector of link direction (-1 or 1). Allows you to
	// change the sign of the joint variable.
	Eigen::Vector<double,DOF> Qsign;

	// @brief Eigen::Matrix4d Tbase: Base transform (between the first and world frame).
	Eigen::Matrix4d Tbase;
	
	// @brief Eigen::Matrix4d Ttool: The tool transform of the robot (between DOF'th frame and tool frame)
	Eigen::Matrix4d Ttool;
	 ...
	 */
	Robot(
		Eigen::Array<double,DOF,4> _DH,
		Eigen::Vector<quik::JOINTTYPE_t,DOF> _linkTypes,
    	Eigen::Vector<double,DOF> _Qsign = (DOF == -1 ? Eigen::VectorXd::Ones(0) : Eigen::VectorXd::Ones(DOF)),
		Eigen::Matrix4d _Tbase = Eigen::Matrix4d::Identity(),
		Eigen::Matrix4d _Ttool = Eigen::Matrix4d::Identity())
		: DH(_DH), linkTypes(_linkTypes), Qsign(_Qsign), Tbase(_Tbase), Ttool(_Ttool)
	{
		...
		if(this->Qsign.size()==0) this->Qsign = Eigen::VectorXd::Ones(this->dof);
	...
	void print() const
	{
		std::cout << "R.linkTypes: [";
		for(int i = 0; i < this->linkTypes.size(); i++){
			if (i!=0) std::cout << ", ";
			if(this->linkTypes(i)) std::cout << "PRISMATIC";
			else std::cout << "REVOLUTE";
		}
		std::cout << "]" << std::endl;
		std::cout << "R.Qsign: " << this->Qsign.transpose() << std::endl;
		std::cout << "R.dof: " << this->dof << std::endl << std::endl << std::endl;
	}
    
	/**
	 * @brief Computes the forward kinematics of the manipulator for a given set of joint angles.
	 * Result is stored in a Eigen::Matrix<double,DOF*4,4> matrix, with each frame transform
	 * stacked vertically ontop of each other.
	 * 
	 * INPUTS:
	 ...
	 */
    void FK( const Eigen::Vector<double,DOF>& Q, Eigen::Matrix<double,(DOF>0?4*(DOF+1):-1),4>& T) const
	{
		// Initialize data
		Eigen::Array<double,1,4> DH_k;
		Eigen::Matrix4d Ak;
	 ...
	 * the final frame as a Eigen::Matrix<double,4,4>. Note, this just calls FK internally.
	 * 
	 * @param[in] Q The input joint angles
	 * @param[out] T The tranformation matrices for the final link
	 */
	void FKn(const Eigen::Vector<double,DOF> &Q, Eigen::Matrix4d& Tn, int frame=-1) const
	{
		constexpr int DOF4 = (DOF>0) ? (DOF+1)*4 : -1;
		Eigen::Matrix<double,DOF4,4> Ti((this->dof+1)*4,4);
		this->FK( Q, Ti );
	 ...
	 * 
	 * @param[in] Q The input joint angles
	 * @param[out] quat Eigen::Vector4d The quaternion out
	 * @param[out] d Eigen::Vector3d the point out
	 * @param[in] frame=-1 int The frame to compute the transform for. Put a negative value for the tool
	 * frame.
	 */
	void FKn(const Eigen::Vector<double,DOF> &Q, Eigen::Vector4d& quat, Eigen::Vector3d& d, int frame=-1) const
	{
	 ...
	 * Default: true.
	 */
	void jacobian( const Eigen::Vector<double, DOF>& Q, Eigen::Matrix<double, 6, DOF>& J, bool includeTool = true) const
	{
		// Initialize the outputs
        constexpr int DOF4 = DOF>0 ? (DOF+1)*4 : -1;
        Eigen::Matrix<double,DOF4,4> T((this->dof+1)*4, 4); // Holds the forward kinematics for each joint
	 ...
	 * @param[out] J The Eigen::Matrix J, passed as reference, in which to store the results.
	 * @param[in] includeTool Whether or not to include the tool transform in the calculations
	 * Default: true.
	 */
	void jacobian( const Eigen::Matrix<double,(DOF>0?4*(DOF+1):-1),4>& T, Eigen::Matrix<double,6,DOF>& J, bool includeTool = true) const
	{
	 ...
	 */
	void hessianProduct( const Eigen::Matrix<double,6,DOF>& J, const Eigen::Vector<double,DOF>& dQ, Eigen::Matrix<double,6,DOF>& A) const
	{
		Eigen::Vector3d cp, jvk, jwk, Aw_k_sum;
	...
	{
		Eigen::Vector<double,DOF> a = DH.col(0);
		Eigen::Vector<double,DOF> d = DH.col(2);
