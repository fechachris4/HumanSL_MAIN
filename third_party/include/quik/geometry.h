...
// REMOVED: using namespace Eigen; - This causes conflicts with GTSAM
// Use explicit Eigen:: prefixes instead
 ...
 */
inline void hgtDiff(const Eigen::Matrix4d& T1, const Eigen::Matrix4d& T2, Eigen::Vector<double,6>& e)
{
    Eigen::Matrix3d R1, R2, Re;
    Eigen::Vector3d d1, d2, eps;
    double eps_norm, t;
 ...
 */
inline Eigen::Matrix4d hgtInv( const Eigen::Matrix4d& T )
{
    Eigen::Matrix4d Tinv;
 ...
 * 
 * @param[in] T Eigen::Matrix4d T, the homogeneous transformation matrix. 
 * @param[out] quat The output quaterneon (x,y,z,w)
 * @param[out] d The output displacement vector (x,y,z)
 */
inline void hgt2quatpos( const Eigen::Matrix4d& T, Eigen::Vector4d& quat, Eigen::Vector3d& d)
{
 ...
 * 
 * @param[in] T Eigen::Matrix<double,Eigen::Dynamic,4> T, the homogeneous transformation matrix. 
 * @param[out] quat Eigen::Matrix<double,4,Eigen::Dynamic> The output quaterneon (x,y,z,w)
 * @param[out] d Eigen::Matrix<double,3,Eigen::Dynamic> The output displacement vector (x,y,z)
 */
inline void hgt2quatpos( const Eigen::Matrix<double,Eigen::Dynamic,4>& T, Eigen::Matrix<double,4,Eigen::Dynamic>& quat, Eigen::Matrix<double,3,Eigen::Dynamic>& d)
{
    ...
    for(int i = 0; i < N; ++i) {
        ...
        Eigen::Vector3d d_i; Eigen::Vector4d quat_i;
 ...
 * @param[out] T Eigen::Matrix4d T, the output homogeneous transformation matrix. 
 */
inline void quatpos2hgt( const Eigen::Vector4d& quat, const Eigen::Vector3d& d, Eigen::Matrix4d& T)
{
    // Convert quaternion to rotation matrix
    Eigen::Quaterniond quaternion(quat(3), quat(0), quat(1), quat(2));
 ...
 * 
 * @param[in] quat Eigen::Matrix<double,4,Eigen::Dynamic> The input quaterneon (x,y,z,w)
 * @param[in] d Eigen::Matrix<double,3,Eigen::Dynamic> The input displacement vector (x,y,z)
 * @param[out] T Eigen::Matrix<double,Eigen::Dynamic,4> T, the output homogeneous transformation matrix. 
 */
inline void quatpos2hgt( const Eigen::Matrix<double,4,Eigen::Dynamic>& quat, const Eigen::Matrix<double,3,Eigen::Dynamic>& d, Eigen::Matrix<double,Eigen::Dynamic,4>& T)
{
    int N = quat.cols();
 ...
 * @return bool
 */
inline bool isRotationMatrix(const Eigen::Matrix3d &R, double tolerance = 1e-6) {

    // Check if is orthogonal (R*R_transpose = I)
    if (! (R * R.transpose()).isApprox(Eigen::Matrix3d::Identity(), tolerance)) return false;
 ...
 * @return bool
 */
inline bool ishgt(const Eigen::Matrix4d &T, double tolerance = 1e-6) {
    // Check if last row is [0, 0, 0, 1]
    if (!T.row(3).transpose().isApprox(Eigen::Vector4d(0, 0, 0, 1), tolerance)) return false;
