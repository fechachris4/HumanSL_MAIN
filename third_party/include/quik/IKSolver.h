...
// REMOVED: using namespace Eigen; and using namespace std; - These cause conflicts with GTSAM
...
public:
            ...
            armijo_beta(_armijo_beta)
    {
        ...
        if(this->max_linear_step_size < 0.05 * this->R->characteristicLength()){
            std::cout << "Warning! The provided max_linear_step_size is very small." << std::endl <<
                ...
                "automatically computed." << std::endl;
        }
     ...
     * 
     * @param[in] Twt Eigen::Matrix4d& Twt: A transformation matrix from the world frame
     * to the tool frame.
     * @param[in] Q0 Eigen::Matrix<double,DOF>& Q0: Initial guesses of the joint angles
     * @param[out] Q_star  Eigen::Matrix<double,DOF>& Qstar: [DOF] The solved joint angles
     * @param[out] e_star Eigen::Matrix<double,6>&e: The pose errors at the final solution.
     ...
     */
    void IK(
		const Eigen::Matrix4d& Twt,
		const Eigen::Vector<double,DOF>& Q0,
		Eigen::Vector<double,DOF>& Q_star,
		Eigen::Vector<double,6>& e_star,
		int& iter,
		quik::BREAKREASON_t& breakReason) const
    {
        // Initialize variables
        Eigen::Vector<double,DOF>  Q = Q0;     // Holds the current solution guess
        Eigen::Vector<double,DOF>  dQ,         // Holds the iterative joint step computed by the algorithm
        ...
        Eigen::Vector<double,6>    e;          // The error vector.
        Eigen::Matrix<double,6,DOF> J(6, this->R->dof), // Holds the robot jacobian
                            A;          // Holds the Hessian product term for the QuIK algorithm
        Eigen::Matrix<double,DOF,DOF> H_i;     // H variable, used in bfgs algorithm
        ...
        Eigen::Matrix<double,DOF4,4> T((this->R->dof+1)*4, 4); // Holds the forward kinematics for each joint
        ...
        for (int i = 0; i < this->max_iterations; i++){
            ...
            switch (this->algorithm){
                ...
                case quik::ALGORITHM_BFGS:
                    ...
                    if (i == 0){
                        H_i = Eigen::Matrix<double,DOF,DOF>::Identity(this->R->dof, this->R->dof);
                    ...
                    if (rho > delta && rho > std::numeric_limits<double>::epsilon())
                        H_i = H_i + ( (1 + delta/rho) * dQ*dQ.transpose() - dQ*y.transpose()*H_i - H_i*y*dQ.transpose())/rho;
                    else if (delta > std::numeric_limits<double>::epsilon() && rho > std::numeric_limits<double>::epsilon())
                ...
                default:
                    // invalid input
                    std::cout << "Invalid algorithm specified!" << std::endl;
                    dQ.fill(0);
     ...
     * 
     * @param[in] quat Eigen::Vector<double,4>& quat: A 4-vector (x,y,z,w), or 4xN matrix of quaternions
     * (one column for each pose to solve).
     * @param[in] d Eigen::Vector<double,3>& d: A 3-vector (x,y,z), or 3xN matrix of displacement
     * vectors (one column for each pose to solve).
     * @param[in] Q0 Eigen::Vector<double,DOF>& Q0: Initial guesses of the joint angles
     * @param[out] Q_star  Eigen::Matrix<double,DOF>& Qstar: [DOF] The solved joint angles
     * @param[out] e_star Eigen::Matrix<double,6>&e: The pose errors at the final solution.
     * @param[out] iter int iter: The number of iterations the algorithm took.
     * @param[out] breakReason BREAKREASON_t breakReason: The reason the algorithm stopped.
     ...
     * 
     * @param[in] Twt Eigen::Matrix<double,Eigen::Dynamic,4>& Twt: A transformation matrix from the world frame
     ...
     * @param[in] Q0 Eigen::Matrix<double,DOF,Eigen::Dynamic>& Q0: Initial guesses of the joint angles
     * @param[out] Q_star  Eigen::Matrix<double,DOF,Eigen::Dynamic>& Qstar: [DOFxN] The solved joint angles
     * @param[out] e_star Eigen::Matrix<double,6,Eigen::Dynamic>&e: The pose errors at the final solution.
     ...
     */
    void IK(
		const Eigen::Matrix<double,Eigen::Dynamic,4>& Twt,
		const Eigen::Matrix<double,DOF,Eigen::Dynamic>& Q0,
		Eigen::Matrix<double,DOF,Eigen::Dynamic>& Q_star,
		Eigen::Matrix<double,6,Eigen::Dynamic>& e_star,
		std::vector<int>& iter,
		std::vector<quik::BREAKREASON_t>& breakReason) const
    {
        ...
        for (int i = 0; i < N; i++){
            ...
            Eigen::Vector<double,DOF> Q_star_i;
     ...
     * and position vector instead of a transformation matrix
     * 
     * @param[in] quat Eigen::Matrix<double,4,Eigen::Dynamic>& quat: A 4-vector (x,y,z,w), or 4xN matrix of quaternions
     * (one column for each pose to solve).
     * @param[in] d Eigen::Matrix<double,3,Eigen::Dynamic>& d: A 3-vector (x,y,z), or 3xN matrix of displacement
		...
		std::vector<quik::BREAKREASON_t>& breakReason) const
    {
        ...
        int N = quat.cols();
        Eigen::Matrix<double,Eigen::Dynamic,4> Twt(N*4,4);
     ...
     * @param e 
     */
    void clampMag(Eigen::Vector<double,6>& e) const
    {
     ...
     */
    void lsolve(
        const Eigen::Matrix<double,6,DOF>& A,
		const Eigen::Vector<double,6>& b,
		Eigen::Vector<double,DOF>& x) const
    {
        // Form covariance matrix
        Eigen::Matrix<double, 6, 6> Astar = A*A.transpose();
        ...
        Eigen::LLT<Eigen::Matrix<double,6,6>> Astar_llt(Astar);
    ...
    {
        std::cout << "\tIKSolver.max_iterations: " << this->max_iterations << std::endl;
        std::cout << "\tIKSolver.algorithm: " << quik::algorithm2str(this->algorithm) << std::endl;
        std::cout << "\tIKSolver.exit_tolerance: " << this->exit_tolerance << std::endl;
        std::cout << "\tIKSolver.minimum_step_size: " << this->minimum_step_size << std::endl;
        std::cout << "\tIKSolver.relative_improvement_tolerance: " << this->relative_improvement_tolerance << std::endl;
        std::cout << "\tIKSolver.max_consecutive_grad_fails: " << this->max_consecutive_grad_fails << std::endl;
        std::cout << "\tIKSolver.lambda_squared: " << this->lambda_squared << std::endl;
        std::cout << "\tIKSolver.max_linear_step_size: " << this->max_linear_step_size << std::endl;
        std::cout << "\tIKSolver.max_angular_step_size: " << this->max_angular_step_size << std::endl;
        if (this->algorithm == quik::ALGORITHM_BFGS){
            std::cout << "\tIKSolver.armijo_sigma: " << this->armijo_sigma << std::endl;
            std::cout << "\tIKSolver.armijo_beta: " << this->armijo_beta << std::endl;
        }
