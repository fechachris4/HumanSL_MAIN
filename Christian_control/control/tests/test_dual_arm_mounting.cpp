...
// pulling yaml-cpp into control for it.
//
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        const std::string origin = ReadScalar(yaml_path, "mount_origin");

        std::printf("mounting YAML: %s\n", yaml_path.c_str());
        std::printf("  base_separation_m %.6f, mount_tilt_rad %.6f, mount_origin %s\n",
        ...
        Check(origin == "base_midpoint",
              "mount_origin is 'base_midpoint' (the only convention this "
        ...
        const pinocchio::SE3& right = model.MountFromBase(Arm::kRight);
        const pinocchio::SE3& left = model.MountFromBase(Arm::kLeft);

        // Position: mirrored about mount's XZ plane, half the separation each
        ...
        // built around, which is where `mount` used to sit. Under the
        // base_midpoint convention there is no base-to-mount radius, so this
        ...
        std::printf("  note: the two bases are %.6f m apart; the apex "
                    ...
                    "(where `mount` sat before 2026-08-05)\n",
