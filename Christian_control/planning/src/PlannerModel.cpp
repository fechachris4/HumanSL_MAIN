...
gtsam::Pose3 DhRootInMount(bool left_arm) {
    // T_mount_dhroot = T_mount_baselink * T_baselink_dhroot. The first
    // factor is read from the URDF through Pinocchio (never hardcoded, so
    // surveying the rig and regenerating the URDF needs no code change
    // here); the second is the DH convention's own pi-about-x offset, which
    // is geometry of the convention rather than of the rig and so is a
    // constant.
