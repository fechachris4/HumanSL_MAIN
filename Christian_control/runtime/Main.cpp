...
void WriteConfigLines(const std::string& log_file, std::ostream& out, const char* prefix) {
    ...
    line("handover_tolerance_m", FormatDouble(config::kHandoverToleranceM));
    line("handover_orientation_tolerance_rad",
         FormatDouble(config::kHandoverOrientationToleranceRad));
    ...
    try {
        ...
        for (std::size_t i = 0; i < arm_count; ++i) {
            ...
            locks.push_back(std::make_unique<ProcessLock>(
                ...
                std::string("another controller process is already controlling ") +
