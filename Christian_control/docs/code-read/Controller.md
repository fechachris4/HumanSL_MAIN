...
**Entry points:** Main.cpp:473 constructs `TrackingController controller(controlled_model);`
before the takeover (so a bad frame name fails early). After that, only the
Runner calls it: `controller.Reset(state)` once at takeover T5 seeding
(Runner.cpp:167), then `controller.DesiredVelocity(state, reference, dt_s,
status)` once per cycle (Runner.cpp:219–221). The destructor runs at Main's
scope exit.

The design rule this file enforces: there is ONE controller. Reference
sources (operator targets, trajectory playback, future Vicon) differ per
run; the controller tracks whichever channel the source filled, and a silent
source means "hold the takeover pose" — so a source that produces nothing
can never cause motion.

---

## Controller.h

### Lines 1–17 — the banner
The routing table: joint channel → joint law (Trajectory.h), pose channel →
reactive law (ReactiveLaw.h), no reference → reactive law toward the pose
captured at `Reset`. It also declares what this file must NOT contain: no
frame conversion, integration, timing, hardware access, printing, or
allocation. That is a real contract — `DesiredVelocity` runs inside the 2 ms
cycle.

### Lines 19–30 — includes and forward declarations
Includes are Eigen plus the two shared-vocabulary headers (ReactiveLaw.h,
State.h). Lines 29–30 are *forward declarations*: `class DualArmKinematics;`
tells the compiler "this type exists" without defining it, which is enough
to hold a reference or a `unique_ptr` to it. The point (stated at line 16)
...
### Lines 32–43 — the public interface
- **39** — `explicit TrackingController(DualArmKinematics& model);`
...
- **40** — a declared destructor. Non-obvious why until you see line 55: the
  ...
  here and *defined* in the .cpp (line 43, `= default`), where the type is
  complete. Delete this line and the header stops compiling for every
  includer, with a confusing "incomplete type" error.
- **44** — `Reset`: captures the CURRENT end-effector pose as the hold pose
  and disarms the arrival notice. Called exactly once per run, at T4/T5.
- **48–51** — `DesiredVelocity`: the per-cycle call. Returns rad/s BEFORE
  clamping (the Runner clamps, the actuation integrates — never this class).
  `status` is an out-parameter for telemetry.

### Lines 53–72 — private state
- **54–55** — the model reference and the owned workspace.
...
- **59–61** — gains and the null-space targets, converted from Config once
...
- **65–66** — the hold pose: position and rotation captured at `Reset`.
  ...
  **[edit-hazard, lines 65–66]** If anything ever called `DesiredVelocity`
...
- **70–72** — the arrival-notice state machine: `last_pose_sequence_` (which
  target we last saw), `pose_sequence_seen_` (have we seen any), and
  `arrival_reported_` defaulting to **true**. **[edit-hazard, lines 70–72]**
...
### Lines 6–10 — includes
`Controller.h` first, then Config (gains), Kinematics (the real
`DualArmKinematics` / `KinematicsWorkspace` definitions — this is the one
translation unit where the controller meets Pinocchio), and Trajectory.h for
`JointTrackVelocity`.

### Lines 12–30 — `ConfiguredGains()` (runs once, from the constructor)
...
the three term switches. **[edit-hazard, lines 16–29]** Note what is *not*
...
### Lines 57–135 — `DesiredVelocity` (once per cycle)
Follow it in execution order:

- **62–66 — the joint channel.** If the reference carries joints
  (`reference.joints` is a `std::optional<JointReference>` — a container
  holding either one value or nothing; in a boolean context it means "is a
  value present?"), return `JointTrackVelocity` immediately: the planner's
  exact joint path, tracked with a wrapped P correction, no FK, no Jacobian.
  **[trajectory-only, lines 64–66]** Only the `TrajectorySource` ever fills
  this channel; operator runs never reach this line. The early return means
  everything below (FK, Jacobian, σ_min, arrival notice) is skipped on
  playback cycles — which is also why the playback CSV's σ_min column is NaN.

- **68–74 — resolve the pose target.** Three-level fallback:
  `reference.pose->p_desired` if the source gave a pose, else the hold
  position; `*reference.pose->rotation` if the pose *also* carries an
  orientation (`rotation` is itself an optional inside the optional —
  "position-only target" is a real case), else the hold rotation. This is
  the line pair that makes "silent source = hold here" true.

- **76–85 — arrival-notice arming.** If a pose is present and its
...
- **87–90 — FK + Jacobian.** The one model call of the cycle, into the
  preallocated workspace. Same adapter as `Reset`: measured right joints +
  fixed left nominal, right 7 columns selected.

- **92–95 — Equation 1, pose error.** `e_pos = p_desired − p_actual`, and
...
- **97–104 — Equation 2, twist error.** The measured end-effector twist is
...
- **106–112 — the arrival check.** If armed and the position error norm
...
- **113–118 — telemetry fill.** Desired/current position, rotation-error
  norm, and the measured tool quaternion. Lines 117–118 flip the whole
  quaternion's sign when w < 0. **[hides-work, lines 116–118]** q and −q are
...
- **120–125 — σ_min.** Builds `J·Jᵀ` (6×6), eigendecomposes it
  ...
  **[mixed-jobs, lines 120–125]** This is pure diagnostics — nothing in the
...
- **127–134 — Equations 3–6.** Copy the gains, scale only the null-space
