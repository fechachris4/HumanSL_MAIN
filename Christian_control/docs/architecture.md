...
- `src/` — application code, one subfolder per technical layer (the layers
  ...
  - `src/control/` — control laws (`Controller.h` — the interface +
    ...
    6-DoF pose controller ported from the simulation; `CylinderRouter` —
    fixed-size end-effector waypoint routing around one vertical cylinder;
...
| `src/control/CylinderRouter.{h,cpp}` | deterministic end-effector waypoint routing around one finite vertical cylinder: direct path when clear; otherwise shortest clockwise/counter-clockwise/over candidate; an inside target is projected outside rather than rejected; fixed-size route storage, pure Eigen geometry, no robot or I/O |
...
| `tests/test_control_logic.cpp` | hardware-free CTest coverage (runs everywhere): `DampedLeastSquares` (incl. singular case), `ClampedCycleDt`, target parsing, `TargetStore`, `PositionIntegration` (seed/integrate/tracking-error wrap), cylinder intersection/routing/inside-target projection/follower |
