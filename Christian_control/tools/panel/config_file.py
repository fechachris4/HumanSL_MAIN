...
Three separate jobs live here because they carry three different permissions.
...
The guard overrides are deliberately absent and are refused by name as well,
because safety policy must not be one click away.
...
# the whitelist already refuses them; they are named again here so the refusal
# says WHY, and so a future edit to KNOBS that added one would fail a test.
...
def write_knob(name: str, value: object, path: Path | None = None) -> tuple[bool, str]:
    ...
    panel started from can always be recovered.
    """
    if name in GUARD_OVERRIDES:
        return False, "guard overrides are not settable from the panel"
