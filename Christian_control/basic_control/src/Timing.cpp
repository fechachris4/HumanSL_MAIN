...
void print_stats(const char* name, std::vector<int64_t> samples_us,
                 std::ostream& out)
{
    ...
    double mean = 0;
    for (int64_t s : samples_us) mean += s;
    mean /= n;

    double var = 0;
    for (int64_t s : samples_us) var += (s - mean) * (s - mean);
                             ...
                             std::ostream& out, int cycles)
{
    ...
    for (int c = 0; c < cycles; ++c) {
        int64_t t0 = now_us();
        k_api::BaseCyclic::Feedback fb = base_cyclic->RefreshFeedback();
                        ...
                        std::ostream& out, int cycles)
{
    ...
    try {
        ...
        print_stats("full control cycle (Refresh: command out + feedback in)",
                    samples, out);
