                         ...
                         const std::atomic<bool>& stop)
{
    ...
    while (!stop) {
        ...
        if (t_now >= next_heartbeat) {
            std::cout << "recording... " << samples << " samples, t="
                      << static_cast<long>(t) << " s\n";
...
{
    csv << "time_s,dt_s";
    for (int i = 1; i <= 7; ++i) csv << ",cmd_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",meas_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",err_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",vel_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",torque_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",current_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",fault_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",warn_j" << i;
...
{
    csv << s.t_s << "," << s.dt_s;
    for (double v : s.commanded_deg)  csv << "," << v;
    for (double v : s.measured_deg)   csv << "," << v;
    for (double v : s.error_deg)      csv << "," << v;
    for (double v : s.velocity_deg_s) csv << "," << v;
    for (double v : s.torque_nm)      csv << "," << v;
    for (double v : s.current_a)      csv << "," << v;
    for (std::uint32_t v : s.fault_bank)   csv << "," << v;
    for (std::uint32_t v : s.warning_bank) csv << "," << v;
    csv << "," << s.arm_state << "," << s.base_fault_bank << ","
        << s.base_warning_bank << "," << (s.refresh_ok ? 1 : 0) << "\n";
}
...
void write_control_trace_header(std::ostream& csv)
{
    ...
    for (int i = 1; i <= 7; ++i) csv << ",leadclip_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",cmd_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",meas_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",vel_j" << i;
    for (int i = 1; i <= 7; ++i) csv << ",fault_j" << i;
    csv << ",arm_state,base_fault,refresh_ok\n";
}

void write_control_trace_row(std::ostream& csv, const ControlTraceSample& s)
{
    csv << s.t_s << "," << s.dt_s;
    ...
    for (bool b : s.speed_clipped) csv << "," << (b ? 1 : 0);
    for (bool b : s.lead_clamped)  csv << "," << (b ? 1 : 0);
    for (double v : s.commanded_deg)  csv << "," << v;
    for (double v : s.measured_deg)   csv << "," << v;
    for (double v : s.velocity_deg_s) csv << "," << v;
    for (std::uint32_t v : s.fault_bank) csv << "," << v;
