//
// Trajectory — implementations for Trajectory.h.
//
...
// ---------------------------------------------------------------
// the file contract: loading, validation, sampling
// ---------------------------------------------------------------
namespace
{
    ...
    // The t_s column carries ~6 significant digits, so at 100 s the
    // quantization is ~1e-4 s. Beyond that is a real grid error.
    constexpr double kTimeGridToleranceS = 1e-4;

    // |finite difference of q - qd| tolerance, deg/s. The producer's qd is
    // analytic, this check is one-sided: they differ by ~0.5*accel*dt
    // (0.3 deg/s at the joint 5-7 limit) plus CSV quantization. 1.0 catches
    // a mislabeled column without false-failing an honest file.
...
Trajectory LoadTrajectoryCsv(std::istream& in, const std::string& name)
{
    ...
    // Knot-spacing bound: execution interpolates LINEARLY at the 500 Hz
    // loop rate (config::kControlDtS = 0.002 s, the upper end of the range
    // below), so the validator's acceleration gate only bounds what is
    ...
    if (dt < 0.0005 - kTimeGridToleranceS || dt > 0.002 + kTimeGridToleranceS)
        Fail(name, "knot spacing " + std::to_string(dt) +
                       ...
                       "500 Hz executor requires loop-rate-scale samples");
...
TrajectorySource::TrajectorySource(Trajectory trajectory,
...
void TrajectorySource::Reset(const RobotState& state)
{
    t_play_s_ = 0.0;
    done_ = false;
...
Reference TrajectorySource::Get(const RobotState& /*state*/, double dt_s,
                                ControllerStatus& status)
{
    ...
    if (refused_)
    {
        ...
        return {}; // no reference: the controller holds the takeover pose
    }
    ...
    Reference reference;
    reference.joints = JointReference{q_ref_now, q_ref_next};
    return reference;
}
