//
// ReactiveLaw: the reactive 6-DoF pose control law — the mathematical policy
// ported from the simulation controller (msc_project
// controller/reactive_controller.py), equations in the same order:
//
//   1. world(base)-frame pose error      e_pos, e_rot = log3(R_des · Rᵀ)
//   2. world(base)-frame twist error     e_v, e_w (zero reference twist)
//   3. task twist                        ẋ = Kp·e_pose + Kd·e_twist
//   4. damped least squares              q̇_task = Jᵀ(JJᵀ + λ²I₆)⁻¹ ẋ
//   5. joint-centering objective         q̇_null = −k_null · wrap(q − q_mid)
//   6. null-space projection             q̇_raw = q̇_task + N q̇_null
//
// Header-only, Eigen-only, fixed-size, no allocation — no robot, no
// Pinocchio — so it is testable in isolation (tests/test_reactive_law.cpp
// cross-validates against the Python simulation law on identical inputs).
//
...
#ifndef HUMANSL_MASTERS_PROJECT_2025_REACTIVELAW_H
...
// Gains and term switches for the reactive pose law. Disabled terms
// contribute exactly zero, so the staged hardware bring-up (P-only first,
// then the velocity term, then null-space centering) is a configuration
...
// AngleAxis returns angle ∈ [0, π], so the result never wraps the long way.
...
// Equation 3: desired world-frame task twist [v; ω] from the pose and twist
// errors. Row order matches the Jacobian: linear first, angular second.
...
// Equation 4: damped least squares for the full 6-DoF task — the 6×7
// counterpart of Dls.h's translational version, same LDLT-on-the-small-
// system construction, no explicit inverse, no allocation.
...
// Equations 5-6: joint-centering velocity projected into the Jacobian null
// space. `centering_mask` is 1 for joints that center, 0 for joints that
// must not (the Gen3's continuous joints 1/3/5/7 have no meaningful
// midpoint). The centering error wraps to (−π, π] — see the header comment.
inline Eigen::Matrix<double, 7, 1>
NullSpaceVelocity(const Eigen::Matrix<double, 6, 7>& jacobian,
                  ...
                  const ReactivePoseGains& gains)
{
    // Equation 5: the centering objective.
...
#endif // HUMANSL_MASTERS_PROJECT_2025_REACTIVELAW_H
