...
namespace
{
    // One number is sent to the robot for each of its seven joints.
    constexpr int NUM_JOINTS = std::tuple_size_v<JointVector>;
    // The controller stores angles in radians; Kortex position commands and
    // the log use degrees, so conversion happens only at the boundary.
...
Actuation::ApplyStatus PositionIntegration::Apply(
