...
void Gen3Arm::make_sdf(
    ...
    auto grid_bisected = PlaneObstacleFromVicon(*grid_with_arm_and_man, arm_base, other_arm_base);
    auto grid_with_tube_and_arm_and_man = TubeObstacleFromVicon(*grid_bisected, tube_info, 0.03);
...
gtsam::Pose3 Gen3Arm::forward_kinematics(gtsam::Pose3& base_pose, 
    ...
    arm_model->fk_model().forwardKinematics(start_conf, {}, joint_poses);
    ee_pose = joint_poses.back();
...
gtsam::Pose3 Gen3Arm::over_head_pose(const Eigen::Vector3d& head_info,
    ...
    if (start_pos.x() - target_x < 0) {
        target_orientation = base_orientation * gtsam::Rot3::Ry(-angle_rad); // -30 degrees about new y-axis
    } else {
        target_orientation = base_orientation * gtsam::Rot3::Ry(angle_rad);  // 30 degrees about new y-axis
    }
...
void Gen3Arm::plan_joint(JointTrajectory& trajectory, 
                     ...
                     int control_frequency, double angle_deg, bool tune_pose, 
    ...
    while(true){
            ...
            try{
                init_values = initJointTrajectoryFromVicon(start_conf, tube_info,
                                                                    offset_from_base_y, offset_from_tube_z,
                                                                    base_pose, total_time_step, best_end_pose, angle_deg, tune_pose);
            }
            catch(...){
                counter++;
                continue;
    ...
    while (execution_ongoing_flag.load()) {
        ...
        if (!replanning_in_progress.load() && !replan_triggered.load()) {
            ...
            if (check_replan_flag.load()) {
                ...
                make_sdf(tube_info_snapshot, human_info_snapshot, true, base_pose_snapshot, other_base_pose_snapshot, other_conf);
                ...
                init_values = 
                        initTaskSpaceTrajectory(start_pose, end_pose, 
                                base_pose, start_conf, pose_trajectory,
                                    percentage, z_offset, y_offset, total_time_step); 
...
void Gen3Arm::plan_cartesian_z(JointTrajectory& trajectory, 
    ...
    gtsam::Pose3 current_ee_pose = forward_kinematics(base_pose, current_joint_pos);
...
void Gen3Arm::plan_cartesian_x(JointTrajectory& trajectory,
    std::vector<double>& current_joint_pos,
    gtsam::Pose3& base_pose,
    const TubeInfo& tube_info,
    double total_time_sec,
    ...
    // Get EE local axes
    gtsam::Point3 x_axis_gtsam = current_ee_pose.rotation().r1();
    gtsam::Point3 z_axis_gtsam = current_ee_pose.rotation().r3();
    Eigen::Vector3d ee_x_axis(x_axis_gtsam.x(), x_axis_gtsam.y(), x_axis_gtsam.z());
    Eigen::Vector3d ee_z_axis(z_axis_gtsam.x(), z_axis_gtsam.y(), z_axis_gtsam.z());

    // Tube axis direction
    Eigen::Vector3d tube_direction = tube_info.direction.normalized();

    // Calculate normal distance between the two skew lines (z-axis and tube axis)
    // For two lines with directions d1, d2 and points p1, p2:
    // normal_distance = |(p2-p1) · (d1 × d2)| / ||d1 × d2||

    // Find a point on the tube axis for the calculation
    // We'll use the centroid as the reference point on the tube axis
    Eigen::Vector3d pos_diff = current_ee_pos - tube_info.centroid;
    Eigen::Vector3d cross_product = ee_z_axis.cross(tube_direction);
    double normal_distance = std::abs(pos_diff.dot(cross_product)) / cross_product.norm();

    // The normal vector between the two skew lines
    Eigen::Vector3d normal_direction = cross_product.normalized();

    // We need to move along x-axis to eliminate this normal distance
    // Project the normal direction onto the EE x-axis to get movement direction
    double distance_to_move = normal_distance;

    // Determine the direction to move along x-axis
    // We want to move in the direction that reduces the normal distance
    double direction_sign = normal_direction.dot(ee_x_axis);

    // If the projection is negative, we need to move in the negative x direction
    if (direction_sign < 0) {
    ...
    // Scale velocities to achieve exact distance
    double scale_factor = std::abs(distance_to_move) / total_distance;
    for (auto& vel : velocities) {
        vel *= scale_factor;
    }
    ...
    for (size_t i = 0; i < total_time_step; ++i) {
        // Update current EE pose based on current joint configuration
        if (i > 0) {
            std::vector<double> current_joint_pos_deg(7);
            for (int j = 0; j < 7; ++j) {
                current_joint_pos_deg[j] = current_joint_conf[j] * 180.0 / M_PI;
            }
            gtsam::Pose3 current_ee_pose_updated = forward_kinematics(base_pose, current_joint_pos_deg);
            gtsam::Point3 x_axis_updated = current_ee_pose_updated.rotation().r1();
            ee_x_axis = Eigen::Vector3d(x_axis_updated.x(), x_axis_updated.y(), x_axis_updated.z());
        }

        // Transform EE x-axis from world frame to base frame
        Eigen::Matrix3d R_base_inv = base_pose.rotation().matrix().transpose(); // Inverse rotation
        Eigen::Vector3d ee_x_axis_base_frame = R_base_inv * ee_x_axis;

        // Desired end-effector velocity along EE x-axis in base frame
        // Apply velocity with correct sign based on distance_to_move
        Eigen::Vector3d desired_ee_vel = velocities[i] * ee_x_axis_base_frame;
...
void Gen3Arm::plan_cartesian_target(JointTrajectory& trajectory,
    std::vector<double>& current_joint_pos,
    gtsam::Pose3& base_pose,
    ...
    // Calculate distance and direction in xz plane
    Eigen::Vector3d current_xz = current_ee_pos;
    current_xz(1) = 0; // Zero out y component to get xz projection

    Eigen::Vector3d target_xz = target_position;
    target_xz(1) = 0; // Zero out y component to get xz projection

    // Calculate distance to target in xz plane
    double distance_to_target = (target_xz - current_xz).norm();

    // Calculate direction from current to target in xz plane
    Eigen::Vector3d direction_xz = (target_xz - current_xz).normalized();
    ...
    // Scale velocities to achieve exact distance
    double scale_factor = distance_to_target / total_distance;
    for (auto& vel : velocities) {
        vel *= scale_factor;
    }
    ...
    for (size_t i = 0; i < total_time_step; ++i) {
        // Transform direction from world frame to base frame
        Eigen::Matrix3d R_base_inv = base_pose.rotation().matrix().transpose(); // Inverse rotation
        Eigen::Vector3d direction_base_frame = R_base_inv * direction_xz;

        // Desired end-effector velocity along direction in base frame
        Eigen::Vector3d desired_ee_vel = -velocities[i] * direction_base_frame;
